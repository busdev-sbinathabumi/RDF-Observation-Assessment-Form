...
import { FormData, Status, FormHeaderData, FollowUpData, ItemInstance } from './types.ts';
import { INITIAL_FORM_SECTIONS_RAW, DEFAULT_RECIPIENT_EMAIL } from './constants.ts';
...
import { SbiLogo } from './components/SbiLogo.tsx';
...
const App: React.FC = () => {
    ...
    const validateForm = useCallback(() => {
        ...
        if (errors.length > 0) {
            ...
            if (hasMissingDescription) alertMessage += '- Please provide a description for all items with "Not OK" status.\n';
            alert(alertMessage);
    ...
    const handleExportToPDF = async () => {
        ...
        setTimeout(async () => {
            ...
            try {
                ...
                const fileName = `Observation_Report_${formData.header.areaLocation.replace(/\s/g, '_')}_${formData.header.assessmentDate}.pdf`;
                pdf.save(fileName);
            } catch (error) {
    ...
    const handleConfirmExportToEmail = () => {
        ...
        const subject = `Area Observation Report - ${header.areaLocation} - ${header.assessmentDate}`;
        
        let body = `Area Observation Report\n\n`;
        ...
        <div id="app-container" className="container mx-auto p-4 sm:p-6 lg:p-8">
            <div className="max-w-4xl mx-auto">
                <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-10">
                    <div>
                        <h1 className="text-2xl sm:text-3xl font-extrabold text-gray-800 tracking-tight">RDF Plant Observation Form</h1>
                        <p className="text-lg text-gray-600">PT Solusi Bangun Indonesia</p>
                    </div>
                    <div className="w-16 h-16"><SbiLogo /></div>
                </header>
                ...
                {displayedSections.map((section) => {
                    ...
                    return (
                        <AssessmentSection
                            ...
                            isOpen={openSectionIndex === originalIndex || openSectionIndex === null}
            ...
            <Modal isOpen={isClearModalOpen} onClose={() => setIsClearModalOpen(false)}>
                <div className="p-4">
                    <h3 className="text-lg font-medium leading-6 text-gray-900">Confirm Form Deletion</h3>
                    <p className="mt-2 text-sm text-gray-500">Are you sure you want to delete all form data? This action cannot be undone.</p>
                    <div className="mt-5 sm:mt-6 flex flex-wrap justify-end gap-3">
                        ...
                        <button type="button" onClick={handleConfirmClear} className="inline-flex justify-center rounded-md border border-transparent bg-red-600 px-4 py-2 text-base font-medium text-white shadow-sm hover:bg-red-700">Delete Form</button>
                    </div>
                </div>
            </Modal>
        </div>
